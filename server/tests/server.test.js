...
describe('GET /todos/:id', ()=>{
  ...
  it('should return todo doc', (done)=>{
    ...
    it('should not return todo doc created by other user', (done)=>{
      console.log(`/todos/${todos[1]._id.toHexString()}`)
      request(app)
      .get(`/todos/${todos[0]._id.toHexString()}`)
      .set('x-auth', users[0].tokens[0].token)
      .expect(404)
...
describe('PATCH /todos/:id', ()=>{
  ...
  it('should not todo created by other  user', (done)=>{
      //grab id of first item
      var hexId=todos[0]._id.toHexString();
      //then make patch request, using url, and id, use send, with request body
      request(app)
...
describe('GET /users/me', ()=>{
  it('should return user if authenticated', (done)=>{
      request(app) //make request using supertest
      .get('/users/me')
      .set('x-auth', users[0].tokens[0].token)
      .expect(200)
      .expect((res)=>{
  ...
  it('should return 401 if not authenticated', (done)=>{
    ...
    .expect(401)
    .expect((res)=>{
      expect(res.body).toEqual({}); //use this to test for empty
    })
    .end(done);
  });
});
...
describe('POST /users', ()=>{
  ...
  it('should return validation errors if request invalid', (done)=>{
      ...
      request(app)
      .post('/users')
      .send({email: 'l@', password: '123'})
      .expect(400)
      .end(done)
  });
  it('should not create user if email in use', (done)=>{
    var email = users[0].email;
    var password = "IamAGoldenGod"
    request(app)
    .post('/users')
    .send({email, password})
    .expect(400)
...
describe('DELETE /users/me/token', ()=>{
    //specify done when asynchronous
